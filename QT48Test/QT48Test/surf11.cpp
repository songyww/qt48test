...
//! Constructor
Surf11::Surf11(IplImage *img, IpVec &ipts)
	: ipts(ipts)
{
	this->img = img;
}
...
void Surf11::getDescriptors(bool upright)
{
...
void Surf11::getUprightDescriptor()
{
	int y, x, count=0;
...
inline float Surf11::gaussian(int x, int y, float sig)
{
...
//! Calculate the value of the 2d gaussian at x,y
inline float Surf11::gaussian(float x, float y, float sig)
{
	return 1.0f/(2.0f*pi*sig*sig) * exp( -(x*x+y*y)/(2.0f*sig*sig));
}
...
inline float Surf11::haarX(int row, int column, int s)
{
...
inline float Surf11::haarY(int row, int column, int s)
{
...
float Surf11::getAngle(float X, float Y)
{
	if(X >= 0 && Y >= 0)
		return atan(Y/X);
